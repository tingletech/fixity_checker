...
class TestCommand(unittest.TestCase):
    ...
    def test_integration(self):
        ...
        self.env.run(command, 'init', '--archive_paths', test_data, '-d', './xx/yy')
        self.env.run(command, 'start', '-d', './xx/yy', expect_stderr=True)
        self.env.run(command, 'status', '-d', './xx/yy', expect_stderr=True)
        self.env.run(command, 'restart', '-d', 'xx/yy', expect_stderr=True)
        self.env.run(command, 'stop', '-d', 'xx/yy', expect_stderr=True)
        self.env.run(command, 'update', 'test-data/README', '-d', 'xx/yy', expect_stderr=True)
